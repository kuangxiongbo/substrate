  ...
  useEffect(() => {
    if (canvasRef.current && imageSrc) {
      ...
      img.onload = () => {
        ...
        // 绘制背景
        ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--cropper-bg') || '#f5f5f5';
        ...
        // 绘制裁剪框边框
        const cropBorderColor = getComputedStyle(document.documentElement).getPropertyValue('--cropper-crop-border') || '#1890ff';
        ...
        // 绘制裁剪框角落标记
        const cornerSize = 20;
        const cornerColor = getComputedStyle(document.documentElement).getPropertyValue('--cropper-corners') || '#1890ff';
